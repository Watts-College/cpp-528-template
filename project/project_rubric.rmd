...
### Portability & renv [10 points, GROUP SCORE] {#port-renv}
...
* Report the R environment and package versions used to create the analysis
    ...
    + 1 (one) student should upload their **final** version of the `renv.lock` file that stores all packages and dependencies. To do this, upload directly on GitHub.com since the file will be ignored by GitHub Desktop per the .gitignore to avoid conflicts between students.
...
* Be kind and include convenient code to install all packages you use in the report
    + Since you’re using `renv` package, you should have a renv.lock file that stores your packages and their dependencies (see **[portability & renv](#port-renv)** section above).
    + Once the final renv.lock is uploaded, just make sure that your team explicitly states that the reader of the report has to use `renv::restore()` to download the specific versions of the packages used in your project.
    + `renv::restore()` is only one line of code but it is powerful!
